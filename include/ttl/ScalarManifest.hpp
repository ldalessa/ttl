...
namespace ttl
{
  ...
  struct ScalarManifest
  {
    ...
    {
      for (int i = 0; Scalar const& s : scalars) {
    ...
    {
      return std::span(begin() + bounds[mask], begin() + bounds[mask + 1]);
    }

    constexpr int find(Scalar const& scalar) const {
      for (int i = 0; i < M; ++i) {
        if (scalar == data[i]) {
          return i;
        }
